...
export const scdUrl = 'https://dev1.scd.oscp.cloudpose.io:5000/scrs';
 ...
 */
export function getContentAtLocation(topic, h3Index, keywords="") {
    if (local) {
    ...
    return request(`${scdUrl}/${topic}?h3Index=${h3Index}&keywords=${keywords}`)
 ...
 * @returns {Promise<SCR> | Promise<string>} Server response Promise
 */
export function getContentWithId(topic, id) {
    if (local) {
    ...
    return request(`${scdUrl}/${topic}/${id}`)
        .then(async (response) => await response.json());
}
 ...
 */
export function searchContentForTenant(topic, token) {
    return request(`${scdUrl}/tenant/${topic}`, GET_METHOD, '', token)
        .then(async (response) => await response.json());
}
 ...
 */
export function postContent(topic, scr, token) {
    if (local) {
    ...
    return request(`${scdUrl}/${topic}`, POST_METHOD, scr, token)
 ...
 */
export async function putContent(topic, scr, id, token) {
    if (local) {
        return Promise.resolve('OK');
    }
    ...
    return request(`${scdUrl}/${topic}/${id}`, PUT_METHOD, scr, token)
        .then(async (response) => await response.text());
}
...
export function deleteWithId(topic, id, token) {
    return request(`${scdUrl}/${topic}/${id}`, DELETE_METHOD, '', token)
        .then(async (response) => await response.text())
}
