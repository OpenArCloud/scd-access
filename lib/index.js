...
export function getContentsAtLocation(url, topic, h3Index, keywords = '') {
    ...
    return request(`${url}/${scrsPath}/${topic}?h3Index=${h3Index}` + keywordsQuery).then(async (response) => await response.json());
}
...
export function getContentWithId(url, topic, id) {
    ...
    return request(`${url}/${scrsPath}/${topic}/${id}`).then(async (response) => await response.json());
}
...
export function searchContentsForTenant(url, topic, token) {
    return request(`${url}/tenant/${scrsPath}/${topic}`, GET_METHOD, '', token).then(async (response) => await response.json());
}
...
export function postContent(url, topic, scr, token) {
    ...
    return request(`${url}/${scrsPath}/${topic}`, POST_METHOD, scr, token).then(async (response) => await response.text());
}
...
export function deleteWithId(url, topic, id, token) {
    return request(`${url}/${scrsPath}/${topic}/${id}`, DELETE_METHOD, '', token).then(async (response) => await response.text());
}
